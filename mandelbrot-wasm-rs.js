...
(function( root, factory ) {
    if( typeof define === "function" && define.amd ) {
        ...
        Rust.mandelbrot_wasm_rs = factory();
    }
}( this, function() {
...
Module.STDWEB_PRIVATE.acquire_tmp = function( dummy ) {
    ...
    if( __load_asynchronously ) {
        return WebAssembly.instantiate( __wasm_module, __imports )
            .then( instance => {
                ...
                console.log( "Finished loading Rust wasm module 'mandelbrot_wasm_rs'" );
                return Module.exports;
            })
            .catch( error => {
                console.log( "Error loading Rust wasm module 'mandelbrot_wasm_rs':", error );
                throw error;
            });
    } else {
    ...
    if( typeof window === "undefined" ) {
        ...
        const wasm_path = path.join( __dirname, "mandelbrot-wasm-rs.wasm" );
        const buffer = fs.readFileSync( wasm_path );
        const mod = new WebAssembly.Module( buffer );

        return __initialize( mod, false );
    } else {
        return fetch( "mandelbrot-wasm-rs.wasm" )
